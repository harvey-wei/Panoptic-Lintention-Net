    ...
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        for in_feature in self.in_features:
            ...
            if head_distance == 0:
                ...
                head_ops["pp_attention"] = Pixel2PatchAttention(in_channels=self.conv_dims,
                                                                num_patches=num_patches,
                                                                query_project=query_project,
                                                                patches_project=patches_project,
                                                                position_embed=position_embed,
                ...
                for i in range(head_distance):
                    ppa_list.append(Pixel2PatchAttention(in_channels=self.conv_dims,
                                                         num_patches=num_patches,
                                                         query_project=query_project,
                                                         patches_project=patches_project,
                                                         position_embed=position_embed,
    ...
    def layers(self, features: Dict[str, torch.Tensor]):
        ...
        for i, f in enumerate(self.in_features):
            if i == 0:
                ...
                feats_reduce = self._modules[f]["channel_reduce"](features[f])

                # feats_reduce shape (N, H, W, C)
                # x shape (N, H, W, C)
                feats_reduce = feats_reduce.permute(0, 2, 3, 1)
                x = self._modules[f]["pp_attention"](feats_reduce)
            else:
                # feats_reduce shape (N, C, H, W)
                feats_reduce = self._modules[f]["channel_reduce"](features[f])
