...
class Pixel2PatchAttention(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        # input shape (N, H, W, C) with C as in_channels
        # output shape (N, H, W, C)
        self.attention = PPAttention(in_channels=in_channels,
    ...
    def forward(self, x):
        ...
        # in_map = x.clone()
        # in_map = in_map.cpu().detach().numpyt()
        # plt.imshow(in_map[0, :, :, 1], cmap='gray')
        # plt.show()
        # out = F.relu(self.norm(x + self.attention(x)))
        # print(f'max act is {out.min():.14f}')
        # # out = out.sum(dim=3)
        # out = out.clone().cpu().detach().numpy()
        # plt.imshow(out[0, :, :, 60] * 256, cmap='gray')
        # plt.show()

        return F.relu(self.norm(x + self.attention(x)))
...
class SemSegHeadSingleHeadPPA2xLin(nn.Module):
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        level_p5 = nn.ModuleDict({
            ...
            "pp_attention": nn.Sequential(Pixel2PatchAttention(self.conv_dims,
                                                               num_patches=num_patches,
                                                               query_project=query_project,
                                                               patches_project=patches_project,
                                                               position_embed=position_embed,
                                          ...
                                          Pixel2PatchAttention(self.conv_dims,
                                                               num_patches=num_patches,
                                                               query_project=query_project,
                                                               patches_project=patches_project,
                                                               position_embed=position_embed,
    ...
    def layers(self, features: Dict[str, torch.Tensor]):
        ...
        # feat_p5 (N, C, H, W)
        feat = None
        for i, f in enumerate(self.in_features):   # i = 0,1,2,3. f = 'p2', 'p3', 'p4', 'p5'
            if i == 0:
                # p2, feat shape  (N, conv_dim, 1/4, 1/4)
                feat = self._modules[f](features[f])
            elif i == 3:
                # feature_reduce (N, 1/32, 1/32, conv_dim)
                feature_reduce = self._modules[f]['channel_reduce'](features[f]).permute(0, 2, 3, 1)

                # feat shape (N, conv_dim, 1/4, 1/4)
                feat += self._modules[f]['pp_attention'](feature_reduce).permute(0, 3, 1, 2)
            else:
                # feat_shape (N, conv_dim, 1/4, 1/4)
                feat += self._modules[f](features[f])
