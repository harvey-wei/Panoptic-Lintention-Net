...
class SemSegHeadPPATest(unittest.TestCase):
    ...
    def test_sem_seg_head_train(self):
        ...
        sim_mat_p5_4 = self.sem_seg_head._modules['p5']['pp_attention'][4]._modules['attention'].sim_mat
        # model._modules['sem_seg_head']._modules['p5']['pp_attention'][4]._modules['attention'].sim_mat
        print(f'{type(sim_mat_p5_4)} and shape {sim_mat_p5_4.size()}')
        torch.save(sim_mat_p5_4, 'sim_mat_p5_4.pt')  # (N, H, W, P)


if __name__ == "__main__":
    # unittest.main()
    sim_mat = torch.load('sim_mat_p5_4.pt')
    print(f'type {type(sim_mat)} shape {sim_mat.size()}')
    att_map_one_sem_group = sim_mat[0, :, :, 2].cpu().detach().numpy()
    import matplotlib.pyplot as plt
    plt.imshow(att_map_one_sem_group, cmap='gray', origin='lower')
    plt.show()
