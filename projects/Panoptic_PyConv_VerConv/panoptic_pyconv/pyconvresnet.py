...
import os
from projects.Panoptic_PyConv.pyconv_util.download_from_url import download_from_url
...
class PyConvBlock(nn.Module):
    expansion = 4   # make the output channels that number of expansion times as large!
...
class PyConvResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False, norm_layer=None, dropout_prob0=0.0):
        ...
        self.inplanes = 64
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)   # take as input the RGB image and havling the size
        self.bn1 = norm_layer(64)
        self.relu = nn.ReLU(inplace=True)

        # PyConvResNet-50 or 101. Their differences lie in the output channel size for each stage.
        # However, other parameters are the same.
        ...
        # Weights of conv2d are initialized according tot kaiming init.
        # while the  weight of BatchNorm or GroupNorm are initialized to be 1
    ...
    def _make_layer(self, block, planes, blocks, stride=1, norm_layer=None, pyconv_kernels=[3], pyconv_groups=[1]):
        # make one stage with `blocks` blocks
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d

        # In total, 4 cases:
        # case 1: both channel size and spatial size are changed.
        # case 2: spatial size is the same but channel size varies.
        # case 3: spatial size is diminished while the channel size is preserved.
        # case 4: both are the same. down-sample layer is set to None.
        downsample = None
        if stride != 1 and self.inplanes != planes * block.expansion:
            # both channel and spatial size are different than those of input.
            downsample = nn.Sequential(
                nn.MaxPool2d(kernel_size=3, stride=stride, padding=1),   # down-sample by a factor of stride
                conv1x1(self.inplanes, planes * block.expansion),
                norm_layer(planes * block.expansion),
            )
        elif self.inplanes != planes * block.expansion:
            # Spatial size is the same but channel size is modified.
            downsample = nn.Sequential(
                conv1x1(self.inplanes, planes * block.expansion),
                norm_layer(planes * block.expansion),
            )
        elif stride != 1:
            # Spatial size is the different but spatial size is modified.
            downsample = nn.MaxPool2d(kernel_size=3, stride=stride, padding=1)

        # For each stage, the first block is responsible for the down-sampling.
        # Only the first block changes the spatial size and channel size.
        layers = []
